...
func TestCreateRSAKey_DefaultBits(t *testing.T) {
	err := pulumi.RunErr(func(ctx *pulumi.Context) error {
		k, err := libkey.CreateRSAKey(ctx, "default", 0)
...
func TestCreateRSAKey_CustomBits(t *testing.T) {
	err := pulumi.RunErr(func(ctx *pulumi.Context) error {
		k, err := libkey.CreateRSAKey(ctx, "custom", 2048)
		require.NoError(t, err)
		require.NotNil(t, k)
