...
func TestRender_Success(t *testing.T) {
	...
	out, err := tpl.Render(path, map[string]string{"Name": "World"})
	require.NoError(t, err)
	assert.Equal(t, "Hello World", out)
}

func TestRender_FileNotFound(t *testing.T) {
	_, err := tpl.Render("/non/existent/path.tmpl", nil)
	require.Error(t, err)
}
...
func TestRender_ParseError(t *testing.T) {
	...
	_, err = tpl.Render(path, map[string]string{"Name": "x"})
	require.Error(t, err)
}

func TestRender_ExecuteTypeError(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "type_err.tmpl")
	err := os.WriteFile(path, []byte("Hello {{ .Name }}"), 0o644)
	require.NoError(t, err)

	_, err = tpl.Render(path, 123)
	require.Error(t, err)
}
